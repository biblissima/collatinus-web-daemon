...
Desinence::Desinence (QString linea)
{
...
AnalyseMorpho::AnalyseMorpho (QString rq, Traits * t, Entree * en, QString ddq)
{
    des = NULL;
    e = en;
    q = rq;
...
Irreg::Irreg (QString linea, Entree * entr)
{
...
Radix::Radix (Entree *entree, int m, int n, QString q, bool ii)
{
    e = entree;
    k = entree->canon ();
    mo = m;
...
QString Entree::defBulle (QString l)
{
    if (trad[l] != NULL)
        ...
        for (int n=1;n<=modelesRad[E->modele ()].count ();++n)
        {
            for (int i = 0; i < E->radicalq (n).size(); ++i)
            {
                ...
                if (!r.isEmpty ())
                {
                    ...
                    radices.insert (r, Radix (E, modelesRad[E->modele ()][n-1], n, rq));
...
void Lexicum::linguamLege (QString lang)
{
...
ListeAnalyses Lexicum::corAnalyses (QString forme)
{
    ...
    for (; i < l; ++i)
    {
        ...
        liste_r.clear ();
        foreach (Radix radix, radices.values (r))
            if ((!radix.en_i () || d.startsWith ("i")) && ((cnt==0)||(cnt == radix.grquant().toLower().count("v"))))
    ...
    {
        e = irr->entree ();
        AnalyseMorpho * am = new AnalyseMorpho (irr->graphieq (), irr->getTraits (), e);
...
QStringList Lexicum::lemmatiseM (QString f, bool mm, bool deb_phr)
{
    ...
    foreach (QString cle, cles)
    {
        ...
        sortie << lin + "<br />"; // saut de ligne entre chaque item (lemme + analyse)
    }
    deleteAnalyses (r);
    return sortie;
}
...
QString Lexicum::lemmatiseTxt (QString txt, bool alpha, bool cumVocibus)
{
    ...
    foreach (QString uox, formes)
    {
        ...
        if (item.at (0) == uox)
        {
            ...
            else lemmes << QString ("<span class=\"echec\">%1</span>").arg (item.at (0));
        }
        else
        {
            if (cumVocibus)
...
QStringList Lexicum::synthA (Entree* e, int c, int n, int g, int d)
{
    ...
    foreach (Desinence des, desinentiae)
    {
            ...
            && des.getTraits ()->gradus () == d)
        {
            ...
            if (Ch::consonnes.contains(des.grquant ()[0]))
                retour << Ch::allonge (r) + des.grquant ();
...
QStringList Lexicum::synthV (Entree* e, int p, int n, int t, int m, int v)
{
    ...
    else // actif
    {
        ...
        for (int j=0;j<ldes.length();++j)
        {
            ...
            {
                if (lr[i] == ".") lr[i] = "";
                if (Ch::consonnes.contains(ldes[j].grquant ()[0]))
                    retours << Ch::allonge (lr[i]) + ldes[j].grquant ();
                else retours << lr[i] + ldes[j].grquant ();
            }
        }
...
QString Lexicum::flechis (Entree* e)
{
    ...
    switch (e->pars ())
    {
        case 'n':
        {
            ...
            ss << "</table>";
            ss << "</div>"; // fin div.table-responsive
            return sortie;
        }
    case 'a':
        {
...
QStringList Lexicum::formeq (QString forme, bool *nonTrouve, bool deb_phr)
{
    QStringList lforme;
    *nonTrouve = true;
    if (forme.isEmpty ())
...
QString Lexicum::scandeTxt (QString texte, bool stats)
{
