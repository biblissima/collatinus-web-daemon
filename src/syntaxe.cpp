...
Mot::Mot (QString data, Lexicum * l, bool expr)
{
    ...
    if (expr)
    {
        ...
        { 
            ...
            {
                e = l->entree (canon);
                analyses.insert (canon, new AnalyseMorpho (canon, new Traits (c,g,n,d,p,t,m,v), e));
            }
        }
    ...
    else
    {
        ...
        analyses = l->lanalyses (gr);
        foreach (QString cle, analyses.keys ())
        {
            entrees << cle;
            foreach (AnalyseMorpho * am, analyses.values (cle))
            {
                canons << am->entree ()->canon ();
            }
        }
        // if (gr.startsWith ("sa")) qDebug () << canons;
...
QString Mot::bulle (QString lang)
{
    // % séparateur d´entrées
    // | séparateur de morphos.
    QString retour;
    QTextStream flux (&retour);
    flux.setCodec("UTF-8");
    flux << gr;
    QString kprec;
    QStringList cles = analyses.keys ();
    ...
    foreach (QString cle, cles)
    {
        ...
        foreach (AnalyseMorpho * am, analyses.values (cle))
        {
            if (prem) flux << "%" << am->entree ()->defBulle (lang);
            prem = false;
            flux << "|" << am->humain ();
        }
    }
    return retour.simplified ();
}

QString Mot::lemmatisation (QString lang)
{
    QString retour;
    retour.append (bulle (lang));
    return retour;
}
...
Phrase::Phrase (QString txt, int d, int f, Lexicum * l)
{
    debut = d;
    fin = f;
    QStringList eclats = txt.split (Ch::reNonAlphas);
    for (int i=0;i<eclats.count();++i)
    {
        if (eclats.at (i).isEmpty ()) continue;
...
bool Phrase::aLeCanon (int pe, Mot * me)
{
    ...
    foreach (Mot * m, mots)
    {
        if (me->getCanons ().empty () || Morph::commun (m->getCanons (), me->getCanons ()))
        {
            ...
            AnalyseMorpho * ame = me->getAnalyses ().values ().at (0); // une seule analyse par mot-expression
...
QStringList Syntaxe::analyse_et_lemmes ()
{
    QStringList retour;
    QString e;
    foreach (Mot *m, phrase->getMots ())
    {
        retour << m->lemmatisation (lexicum->lang ());
    }
    return retour;
}
...
QStringList Syntaxe::lemmatisation (bool alpha)
{
    ...
    foreach (Mot * m, phrase->getMots ()) 
    {
        ...
        if (m->getCanons ().count () == 0) // échec de la lemmatisation
        {
            ...
            {
                retour << QString ("<span class=\"echec\">%1</span><br/>").arg (m->graphie ());
            }
        ...
        {
            exprPhr (c);
            lemMot << m->lemmatisation (lexicum->lang ());
        }
        lemMot.removeDuplicates ();
        if (!m->getExpression ().isEmpty ())
            lemMot.last ().append ("%"+m->getExpression ());
        if (!lemMot.empty ())
            lemMot.last ().append ("//");
        else retour.append ("//");
        retour << lemMot;
